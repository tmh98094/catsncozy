...
import { ArrowLeft, Heart, Check, Image as ImageIcon } from 'lucide-react';
import { Cat, AdoptionFormData, ViewState } from '../types';
...
const Adoption: React.FC<AdoptionProps> = ({ onBack, onNavigate, cats }) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const cardsRef = useRef<HTMLDivElement>(null);

  const [selectedCat, setSelectedCat] = useState<Cat | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedCatForGallery, setSelectedCatForGallery] = useState<Cat | null>(null);
...
`;
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-cat-yellow w-full pb-32 md:pb-20">
      {/* Consistent Navbar */}
      <SubPageNav onBack={onBack} onNavigate={onNavigate} currentPage="adopt" />

      {/* Header - Add top padding for fixed navbar */}
      <header className="sticky top-16 z-40 bg-cat-yellow/90 backdrop-blur-sm border-b-4 border-cat-black p-4 md:p-6 flex items-center justify-center adopt-header transition-all mt-14">
          ...
          <div key={cat.id} className="cat-card group bg-white border-4 border-cat-black rounded-3xl overflow-hidden hover:shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] transition-all duration-300 transform hover:-translate-y-2 flex flex-col">
            <div className="relative h-72 overflow-hidden border-b-4 border-cat-black">
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setSelectedCatForGallery(cat);
                  setIsGalleryOpen(true);
                }}
                className="absolute top-4 left-4 bg-white/90 backdrop-blur p-2 rounded-full border-2 border-cat-black hover:bg-cat-black hover:text-white transition-colors z-10 shadow-md"
                title="View Gallery"
              >
                <ImageIcon size={18} />
              </button>
      ...
      <Modal
        ...
        title={`${selectedCatForGallery?.name}'s Album`}
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {(selectedCatForGallery?.images || []).map((img, idx) => (
            <div key={idx} className="rounded-xl overflow-hidden border-2 border-cat-black aspect-square bg-gray-100 relative group">
              <img src={img as string} alt="Gallery" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
            </div>
          ))}
        </div>
        {(!selectedCatForGallery?.images || selectedCatForGallery?.images.length === 0) && !selectedCatForGallery?.image && (
          <p className="text-center text-gray-500 py-8">No images available for this cat.</p>
        )}
      </Modal>
    </div >
  );
